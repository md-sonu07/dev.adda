...
const ArticleCard = ({ article, formatTime, fetchPriority = "auto" }) => {
    ...
    const handleCardClick = () => {
        if (isAd) return; // Don't navigate for ads if they are just placeholders
    ...
    return (
        <div
            ...
            className={`group flex flex-col sm:flex-row bg-card rounded-2xl border border-default overflow-hidden transition-all duration-300 h-auto sm:h-72 shadow-sm ${isAd ? 'hover:border-red-500/30' : 'hover:border-primary/50 cursor-pointer'}`}
        >
                ...
                <div className="flex items-center gap-2 font-display text-body">
                    {isAd ? (
                        <button
                            onClick={(e) => {
                                e.stopPropagation();
                                toast("Upgrade to a premium plan to remove all ads!", { icon: '🚀' });
                                navigate("/payment");
                            }}
                            className="flex items-center cursor-pointer gap-2 px-6 py-2 rounded-xl bg-red-500/10 hover:bg-red-500/20 transition-all text-[10px] font-black uppercase tracking-widest text-red-500 border border-red-500/20 group/remove"
                        >
                            <HiOutlineXMark className="text-base group-hover:rotate-90 transition-transform" />
